...
<body style="min-height: 800px; overflow: scroll;background-color: #f5f5f5;width: 100%;overflow-x: hidden">
    ...
    <div class="main_div" style="">
        <div class="container">
            @if (Session::has('flash_message_sucess'))
                <div class="alert alert-success alert-dismissible fade show" id="sucess_message" role="alert">
                    ...
                    <button type="button" class="close close_flash" data-bs-dismiss="alert" aria-label="Close">
        ...
        <div id="splashPopup" class="splash-popup" style="display: none;">
            <div class="splash-popup-content">
                <span class="close-btn" onclick="closePopup()">&times;</span>
                <img id="splashImage" src="" alt="Splash Image" />
                <p id="splashDescription"></p>
                <a id="splashLink" href="#" class="btn btn-primary">Learn More</a>
            </div>
        </div>
        <style>
            .splash-popup {
...
<script>
    function showPopup(image, description, link) {
        document.getElementById('splashImage').src = image;
        document.getElementById('splashDescription').innerText = description;
        document.getElementById('splashLink').href = link;
        document.getElementById('splashPopup').style.display = 'flex';
    }

    function closePopup() {
        document.getElementById('splashPopup').style.display = 'none';
    }

    // Example: Show the popup after the page loads
    @if($splashPopUp)
        window.onload = function() {
            let imageUrl = '{{ asset($splashPopUp->image) }}'; // Dynamic image path
            let description = '{{ $splashPopUp->description }}';
            let link = '{{ $splashPopUp->url }}';
            showPopup(imageUrl, description, link);
        };
    @endif
</script>
